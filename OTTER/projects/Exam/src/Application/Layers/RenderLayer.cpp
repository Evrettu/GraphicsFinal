...
void RenderLayer::OnAppLoad(const nlohmann::json& config)
{
	...
	_lightAccumulationShader->SetDebugName("Light Accumulation");
	...
	_lightAccumulationShader->SetUniform("u_Diffuse", 1.0f);
	_lightAccumulationShader->SetUniform("u_Specular", 1.0f);
...
const ShaderProgram::Sptr RenderLayer::GetLightAccumulationShader() const
{
	return _lightAccumulationShader;
}
