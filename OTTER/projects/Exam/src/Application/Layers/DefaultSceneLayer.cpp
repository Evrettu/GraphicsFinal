...
#pragma endregion 
		...
		// Loading in a 1D LUT
		Texture1D::Sptr toonLut = ResourceManager::CreateAsset<Texture1D>("luts/toon-1.png"); 
		...
		ShaderProgram::Sptr      redShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/skybox_vert.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/red.glsl" }
		});
		...
		GameObject::Sptr player = scene->CreateGameObject("player");
		{
			...
			player->SetPostion(glm::vec3(1.5f, 4.0f, 1.0f));

			// Add some behaviour that relies on the physics body
			player->Add<JumpBehaviour>();
			RigidBody::Sptr physics = player->Add<RigidBody>(RigidBodyType::Dynamic);
			physics->AddCollider(ConvexMeshCollider::Create())->SetScale(glm::vec3(1.0f));
		...
		{
			// Set position in the scene
			Enemy->SetPostion(glm::vec3(-15.0f, 4.0f, 1.0f));
			Enemy->SetScale(glm::vec3(0.1f));

			// Add some behaviour that relies on the physics body
			Enemy->Add<ShipMoveBehaviour>();
			RigidBody::Sptr physics = Enemy->Add<RigidBody>(RigidBodyType::Dynamic);
			physics->AddCollider(ConvexMeshCollider::Create())->SetScale(glm::vec3(0.1f));
		...
		GameObject::Sptr trigger = scene->CreateGameObject("Trigger");
		{
			...
			collider->SetPosition(glm::vec3(-15.0f, 5.0f, 0.5f));
		...
		{
			Smokeparticles->SetPostion({ -15.0f, 5.0f, 0.5f });
		...
		{
			// Set position in the scene
			shadowCaster->SetPostion(glm::vec3(0.0f, 15.0f, 7.0f));
